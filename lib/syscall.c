...
int read (int fd, uint8_t *buffer, int size) {
	//DONE: Complete the function 'read' just like the function 'open'.
	return syscall(SYS_READ, fd, (uint32_t)buffer, size, 0, 0);
}

int lseek (int fd, int offset, int whence) {
	//DONE: Complete the function 'lseek' just like the function 'open'.
	return syscall(SYS_LSEEK, fd, offset, whence, 0, 0);
}

int close (int fd) {
	//DONE: Complete the function 'close' just like the function 'open'.
