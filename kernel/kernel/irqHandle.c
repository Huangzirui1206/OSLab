...
// vga functions only handle displayRow and displayCol 
inline void vgaNewLine(){
	displayCol=0;
	displayRow++;
	if(displayRow==25){
		scrollScreen();
...
inline void vgaPutChar(char c){
	uint16_t data = c | (0x0c << 8);
	uint32_t pos = (80*displayRow+displayCol)*2;
	asm volatile("movw %0, (%1)"::"r"(data),"r"(pos+0xb8000));
	displayCol+=1;
}

inline void vgaBackSpace(){
	displayCol--;
...
void irqHandle(struct TrapFrame *tf) { // pointer tf = esp
	...
	switch(tf->irq) {
		// TODO: 填好中断处理程序的调用
		case -1: break;	//I can't understand why, but it really works. Maybe the idt we set is not comprehensive. The irq is 46 and 48
