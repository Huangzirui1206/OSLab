...
int scanf(const char *format,...) {
	...
	while(format[i]!=0){
		if(buffer[count]==0){
			...
			count=0;
			if(ret >= MAX_BUFFER_SIZE)buffer[count] = 0;
		}
		switch(state){
			case 0:
				switch(format[i]){
...
// Visit critical area
int read_rcnt(){
	return syscall(SYS_CRI_MEM, CR_RCNT_R,0,0,0,0);
}
int inc_rcnt(){
	return syscall(SYS_CRI_MEM, CR_RCNT_INC,0,0,0,0);
}
int dec_rcnt(){
	return syscall(SYS_CRI_MEM, CR_RCNT_DEC,0,0,0,0);
}
int read_wcnt(){
	return syscall(SYS_CRI_MEM, CR_WCNT_R,0,0,0,0);
}
int inc_wcnt(){
	return syscall(SYS_CRI_MEM, CR_WCNT_INC,0,0,0,0);
}
int dec_wcnt(){
	return syscall(SYS_CRI_MEM, CR_WCNT_DEC,0,0,0,0);
}
int write_file(const char* src){
	return syscall(SYS_CRI_MEM, CR_FILE_W,(uint32_t)src,0,0,0);
}
int read_file(char* dst){
	return syscall(SYS_CRI_MEM, CR_FILE_R,(uint32_t)dst,0,0,0);
}