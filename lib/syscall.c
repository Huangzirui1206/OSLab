...
char getChar(){ // 对应SYS_READ STD_IN
	...
	return (char)syscall(SYS_READ, STD_IN, 0, 0, 0, 0);	//eax = (SYS_READ), ecx = (STD_IN)
}

void getStr(char *str, int size){ // 对应SYS_READ STD_STR
	// TODO: 实现getStr函数，方式不限
	syscall(SYS_READ, STD_STR, (uint32_t)str, size, 0, 0); //eax = SYS_READ), ecx = (STD_STR), edx = (str), ebx = (size)
}
...
void printf(const char *format,...){
	...
	while(format[i]!=0){
		...
		switch(state){
			case 0:{
				if(format[i]!='%')buffer[count++] = format[i++];
				else {
					state = 1;
					++i;
				}
			...
			case 1:{
				switch(format[i++]){
					...
					case 'c':{
						buffer[count++] =(char)((int*)paraList)[++index];
						break;
					}
					case 's':{
			...
			default: /*Don't need to handle illegal situations here.*/
				return;
		}
	}
	if(count!=0)
			...
			case 1:{
				switch(format[i++]){
					case 'd': {
						...
						*((int**)paraList)[++index] = str2Int(buffer);
						break;
					}
					case 'c':{
						*((char**)paraList)[++index] = getChar();
						break;
					}
					case 's':{
						getStr(((char**)paraList)[++index],MAX_BUFFER_SIZE);	
						break;
					}
